...
Lyman-$\alpha$ is ideal as a target to study the exosphere, as these photons are excited by resonant scattering from atmospheric H and produce very bright emissions that can only be observed by spacecraft.
...
This paper focuses on the region of the exosphere where H density is low enough that most solar photons scatter once before entering the detector, known as the optically thin regime.  Details of the reconstruction process for the optically thick regime can be found in (FIXME: citation)

  Knowledge of the H density distribution in the exosphere has advanced primarily due to spaceborne (UV) sensing.  While direct measurement is possible with mass spectrometers (e.g. sounding rockets), the sheer size of the exosphere makes this approach impractical.  Ground-based remote sensing has been hampered by the fact that O₂ at lower altitudes is opaque to (UV) and by limited measurement angles due to a fixed sensor platform.
...
\begin{figure*}[h!]
  \centering
  \includegraphics[height=2.7in]{figures/previous_missions}
...
\label{sec:measurement_constraints}
  ...
  Carruthers was inserted into a halo orbit around the L1 Lagrange point (about 1.5 million km from Earth, 235 Re), which is distant enough to observe the entirety of the geocorona from an outside vantage
...
A tenuous distribution of hydrogen throughout the solar system, known as interplanetary hydrogen (IPH), also contributes to the line-integrated Lyman-$\alpha$ radiance detected by the spacecraft.
  IPH, like solar g factor, will be measured independently by Carruthers (FIXME: cite GISSMO?) and is assumed to be known.
...
\end{figure}
  ...
  The final step of post-processing is to bin the native 512² pixel WFI and 1024² pixel NFI images to a lower resolution
...
Choosing a discretization grid which is well-suited to the data is critical, as an improper grid can create aliasing and other artifacts that cause the numerical result to deviate from the functions they approximate.  Since the exosphere is well-understood to smoothly vary with larger density gradients at lower altitudes, a regular spherical grid with logarithmically spaced radial bins is appropriate.  As opposed to more sophisticated gridding schemes, regular grids allows for the use of a simple multidimensional array as the underlying data structure, and the property of shared boundaries between voxels simplifies tomography calculations, as demonstrated in Tomosphero (FIXME: introduce tomosphero earlier) \cite{tomosphero} (FIXME: tomosphero citation).}
    ...
    This manuscript uses notation convention $r$, $e$, $a$ when referring to radial, elevational, and azimuthal dimensions to avoid ambiguity with astrophysical versus mathematical conventions for spherical coordinates as in Table \ref{tab:sph_convention} that denote the elevational and azimuthal dimensions differently.
...
\begin{table}[h]
  \centering
  \setlength{\tabcolsep}{4pt} % Default value: 6pt
  \caption{Spherical coordinate notation.}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    \thead{Dimension} &
